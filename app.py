...
def save_document_data(data):
    ...
    try:
        ...
        for doc_id, doc in data.items():
            save_data[doc_id] = doc.copy()
            if "chunk_embeddings" in save_data[doc_id]:
                save_data[doc_id]["chunk_embeddings"] = [emb.tolist() for emb in doc["chunk_embeddings"]]
                ...
                if block.get("type") == 0:  # Text block
                    for line in block.get("lines", []):
                        ...
                        if text and re.match(r"^[A-Z][A-Za-z0-9 \-:]+$", text):
                            headings.append({"text": text, "y0": line.get("bbox", [0,0,0,0])[1]})
        ...
        for slide_num, slide in enumerate(prs.slides, 1):
            ...
            for shape in slide.shapes:
                if hasattr(shape, "text") and shape.text.strip():
                    ...
                    # Check if shape is a title or other heading placeholder
                    if (hasattr(shape, "is_placeholder") and shape.is_placeholder):
                        headings.append({"text": shape.text.strip(), "slide": slide_num})
            text += slide_text + "\n"
    ...
    try:
        ...
        for para in doc.paragraphs:
            if para.style.name and para.style.name.startswith('Heading'):
                headings.append({"text": para.text.strip(), "level": para.style.name})
            text += para.text + "\n"
...
def extract_headings_and_content(pdf_url=None, raw_text=None, filepath=None, heading_pattern=r"^(#+)?\s*([A-Z][A-Za-z0-9 \-:]+)$"):
    ...
    if filepath:
        ...
        if file_ext == 'pdf':
            ...
            chunks = {h["text"]: "" for h in headings_list}
...
def query_ollama(prompt):
    ...
    try:
        ...
        print("Received Ollama API response")
        return result
        
    except Exception as e:
        print(f"Ollama API query failed: {str(e)}")
    ...
    try:
        ...
        for doc_id in document_ids:
            if doc_id in document_chunks:
                doc_data = document_chunks[doc_id]
                all_chunks.extend(doc_data.get('chunks', []))
                all_chunk_embeddings.extend(doc_data.get('chunk_embeddings', []))
                all_images.extend(doc_data.get('images', []))
            else:
                    ...
                    for img in all_images:
                        ...
                        if img.get("heading") == heading:
                            try:
                                ...
                                relevant_images.append({
                                    "url": f"/images/{img.get('filename', '')}",
                                    "heading": img.get("heading", "Unknown"),
                                    "similarity": 0.0
                                })
        
        # Fallback to chunk-based retrieval if no headings found
        if not relevant_excerpts:
            print("Using chunk-based retrieval fallback...")
            relevant_chunks = get_relevant_chunks(user_message, all_chunks, all_chunk_embeddings)
            relevant_excerpts = [f"### Excerpt\n{chunk[0][:500]}" for chunk in relevant_chunks]
            
            # Find images near relevant chunks
            for chunk, similarity in relevant_chunks:
                for img in all_images:
                    if not isinstance(img, dict):
                        continue
                    if any(keyword.lower() in img.get("heading", "").lower() for keyword in user_message.split()):
                        relevant_images.append({
                            "url": f"/images/{img.get('filename', '')}",
                            "heading": img.get("heading", "Unknown"),
                            "similarity": float(similarity)
        ...
        prompt = f"""You are an AI assistant helping with document analysis. Answer the user's question based on the provided context.
...
5. Format your response clearly with paragraphs and bullet points when helpful

Answer:"""
        
        print(f"Sending prompt to Ollama with {len(relevant_excerpts)} excerpts and {len(relevant_images)} images")
